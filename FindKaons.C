...
void FindKaons() {
    ...
    // Mass plots
    //
    TH1F * hKaonMass_0_20 = new TH1F("hKaonMass_0_20", "#pi^{+}#pi^{-} mass distribution", 100, 0.44, 0.56);
    hKaonMass_0_20->GetXaxis()->SetTitle("M_{#pi^{+}#pi^{-}(GeV)}"); hKaonMass_0_20->GetYaxis()->SetTitle("Counts");
    TH1F * hKaonMass_20_40 = new TH1F("hKaonMass_20_40", "#pi^{+} #pi^{-} pairs", 100, 0.44, 0.56);
    TH1F * hKaonMass_40_60 = new TH1F("hKaonMass_40_60", "#pi^{+} #pi^{-} pairs", 100, 0.44, 0.56);
    TH1F * hKaonMass_60_80 = new TH1F("hKaonMass_60_80", "#pi^{+} #pi^{-} pairs", 100, 0.44, 0.56);
    TH1F * hKaonMass_80_100 = new TH1F("hKaonMass_80_100", "#pi^{+} #pi^{-} pairs", 100, 0.44, 0.56);
    TH1F * hKaonMass_100_150 = new TH1F("hKaonMass_100_150", "#pi^{+} #pi^{-} pairs", 100, 0.44, 0.56);
    hKaonMass_100_150->GetXaxis()->SetTitle("M_{#pi^{+}#pi^{-}(GeV)}"); hKaonMass_100_150->GetYaxis()->SetTitle("Counts");
    TH1F * hKaonMass_150_200 = new TH1F("hKaonMass_150_200", "#pi^{+} #pi^{-} pairs", 100, 0.44, 0.56);
    TH1F * hKaonMass_200_300 = new TH1F("hKaonMass_200_300", "#pi^{+} #pi^{-} pairs", 100, 0.44, 0.56);
    TH1F * hKaonMass_300_500 = new TH1F("hKaonMass_300_500", "#pi^{+} #pi^{-} pairs", 100, 0.44, 0.56);
    TH1F * hKaonMass_500_1000 = new TH1F("hKaonMass_500_1000", "#pi^{+} #pi^{-} pairs", 100, 0.44, 0.56);
    TH1F * hMpiMass_80_150MeV = new TH1F("hMpiMass_80_150MeV", "#pi^{+}#pi^{-} mass distribution P_{T} < 100 MeV",
                                                                                        100, 0.44, 0.56);
    hMpiMass_80_150MeV->GetXaxis()->SetTitle("M_{#pi^{+}#pi^{-}}(GeV)"); hMpiMass_80_150MeV->GetYaxis()->SetTitle("dN/dM_{#pi^{+}#pi^{-}}(GeV)");

    TH1F * hKaonMass = new TH1F("hKaonMass", "#pi^{+} #pi^{-} pairs", 200, 0.44, 0.56);
    ...
    //
    TLorentzVector pTrackMomentum; TLorentzVector nTrackMomentum; TLorentzVector TrackMomentum;
    TLorentzVector KaonMomentum; TLorentzVector Polarization;
    ...
    while (myReader.Next() && iEvent < EventLimit) {
        ...
        for (unsigned int iTp = 0; iTp < TotTracks; iTp++ ){
            ...
            for (unsigned int iTn = 0; iTn < TotTracks; iTn++ ){
                ...
                KaonMomentum = nTrackMomentum + pTrackMomentum;
                Polarization = nTrackMomentum - pTrackMomentum;
                float DeltaPhi = KaonMomentum.Phi() - Polarization.Phi();
                float KaonMass = KaonMomentum.M();
                RhoMsqr = GetMassSqr(KaonMomentum.P(), trackBeta[iTn]);
                DeltaTOF_expected = GetMassSqr(KaonMomentum.P(), trackBeta[iTn]);
                hXpionXkaon->Fill(NSigmaKaon, NSigmaPion);
                hXpionXee->Fill(NSigmaElectron, NSigmaPion);
                hXpionXpp->Fill(NSigmaProton, NSigmaPion);
                hMassVsPt->Fill(KaonMomentum.Pt(), KaonMass);
                if (KaonMomentum.Pt()<0.02){
                    hKaonMass_0_20->Fill(KaonMass);
                }
                if (KaonMomentum.Pt()>0.02 && KaonMomentum.Pt()<0.04){
                    hKaonMass_20_40->Fill(KaonMass);
                }
                if (KaonMomentum.Pt()>0.04 && KaonMomentum.Pt()<0.06){
                    hKaonMass_40_60->Fill(KaonMass);
                }
                if (KaonMomentum.Pt()>0.06 && KaonMomentum.Pt()<0.08){
                    hKaonMass_60_80->Fill(KaonMass);
                }
                if (KaonMomentum.Pt()>0.08 && KaonMomentum.Pt()<0.1){
                    hKaonMass_80_100->Fill(KaonMass);
                }
                if (KaonMomentum.Pt()>0.1 && KaonMomentum.Pt()<0.15){
                    hKaonMass_100_150->Fill(KaonMass);
                }
                if (KaonMomentum.Pt()>0.15 && KaonMomentum.Pt()<0.2){
                    hKaonMass_150_200->Fill(KaonMass);
                }
                if (KaonMomentum.Pt()>0.2 && KaonMomentum.Pt()<0.3){
                    hKaonMass_200_300->Fill(KaonMass);
                }
                if (KaonMomentum.Pt()>0.3 && KaonMomentum.Pt()<0.5){
                    hKaonMass_300_500->Fill(KaonMass);
                }
                if (KaonMomentum.Pt()>0.5 && KaonMomentum.Pt()<1){
                    hKaonMass_500_1000->Fill(KaonMass);
                }
                if (KaonMomentum.Pt()>0.8 && KaonMomentum.Pt()<0.150){
                    hMpiMass_80_150MeV->Fill(KaonMass);
                    hKaonMass->Fill(KaonMass);
                }
                if ((DeltaDeltaTOF < 0.750 && DeltaDeltaTOF > 0) && NSigmaPion < 8) {
                    hKaonMass->Fill(KaonMass);
                    if (KaonMass > 0.65 && KaonMass < 0.90 
                        // && fabs(KaonMomentum.Rapidity()) >= 0.6
                        // && fabs(KaonMomentum.Rapidity()) <= 1.0
                        ) {
                        // Defining new reference frame variables.
                        cosphi = (KaonMomentum.Px()*Polarization.Px() + KaonMomentum.Py()*
                               Polarization.Py())/(KaonMomentum.Pt()*Polarization.Pt());
                        sinphi = (KaonMomentum.Px()*Polarization.Py() - KaonMomentum.Py()*
                               Polarization.Px())/(KaonMomentum.Pt()*Polarization.Pt());
                        float phi = TMath::ACos(cosphi);
                        if (KaonMomentum.Pt()*sinphi< 0){
                            phi = -1*phi;
                        }
                        ...
                        // Histograms
                        // 2D histogram
                        hPxPy->Fill(KaonMomentum.Pt()*cos(phi), KaonMomentum.Pt()*sin(phi));
                        hCos2phivsPT->Fill(KaonMomentum.Pt(), 2*cos(2*phi));

                        // 1D Histograms 
                        hDeltaPhi->Fill(DeltaPhi);
                        hPhipPPhim->Fill(KaonMomentum.Phi());
                        hPhipMPhim->Fill(Polarization.Phi());
                        hcosphi->Fill(cosphi);
                        hPhip->Fill(pTrackMomentum.Phi());
                        hPhim->Fill(nTrackMomentum.Phi());
                        hRhoTP->Fill(KaonMomentum.Pt());
                        if (KaonMomentum.Pt()<0.060) {
                           hphi->Fill(phi);
                           hCos2phivsY->Fill(abs(KaonMomentum.Rapidity()), 2*cos(2*phi));
    ...
    hKaonMass_0_20->Scale(1.0 / hKaonMass_0_20->Integral());
    hKaonMass_20_40->Scale(1.0 / hKaonMass_20_40->Integral());
    hKaonMass_40_60->Scale(1.0 / hKaonMass_40_60->Integral());
    hKaonMass_60_80->Scale(1.0 / hKaonMass_60_80->Integral());
    hKaonMass_80_100->Scale(1.0 / hKaonMass_80_100->Integral());
    hKaonMass_100_150->Scale(1.0 / hKaonMass_100_150->Integral());
    hKaonMass_150_200->Scale(1.0 / hKaonMass_150_200->Integral());
    hKaonMass_200_300->Scale(1.0 / hKaonMass_200_300->Integral());
    hKaonMass_300_500->Scale(1.0 / hKaonMass_300_500->Integral());
    hKaonMass_500_1000->Scale(1.0 / hKaonMass_500_1000->Integral());

    normalizeHistogramProbability(hMpiMass_80_150MeV);
